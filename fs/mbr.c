...
void mbr_setup_parttable(drive_t *drive){
   ...
   char *buffer = k_malloc( drive->drive_block_size, 0 );
   drive->drive_read_lba( drive, buffer, 0, 1 );

   mbr_info_t *mbr_info = k_malloc(sizeof(mbr_info_t), 0);
   memcpy(mbr_info, buffer + MBR_PARTINFO_START, sizeof(mbr_part_t)*4);
   drive->partition_data = mbr_info;

   //Setup functions for accessing MBR data
   drive->drive_part_lba_start = mbr_get_partstart_lba;
   drive->drive_part_size = mbr_get_partsize;
}

uint32_t mbr_get_partstart_lba(drive_t *drive, int part_number){
   //Subtract 1 since partitions numbers start at 1
   return ((mbr_info_t*)drive->partition_data)->mbr_partitions[part_number-1].start_lba;
}

uint32_t mbr_get_partsize(drive_t *drive, int part_number){
   return ((mbr_info_t*)drive->partition_data)->mbr_partitions[part_number-1].total_sectors;
}
