...
void irq_enable(uint8_t num){
   if( num <= PIC_INTERRUPT_LINES ){
      int8_t current_mask = portb_read(MASTER_PIC_DATA_P);

      //We enable an IRQ by setting the corresponding bit to 0
      int8_t mask = ~(0x1 << num);
      portb_write( MASTER_PIC_DATA_P, current_mask & mask );
   }else{
      portb_write( SLAVE_PIC_DATA_P, 0 );
      int8_t current_mask = portb_read(SLAVE_PIC_DATA_P);
      int8_t mask = ~(0x1 << (num - PIC_INTERRUPT_LINES) );
      portb_write( SLAVE_PIC_DATA_P, current_mask & mask );
   }
}
